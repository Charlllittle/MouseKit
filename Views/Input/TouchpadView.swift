...
struct TouchpadView: View {
  ...
  private func handleTwoFingerPanChange(_ translation: CGSize) {
    // Translation is already the delta (reset after each update)
    gestureProcessor.handleScrollChange(translation: translation) {
      verticalDelta, horizontalDelta in
...
struct AllGesturesView: UIViewRepresentable {
  ...
  func makeUIView(context: Context) -> UIView {
    ...
    let twoFingerTap = UITapGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.handleTwoFingerTap))
    twoFingerTap.numberOfTouchesRequired = 2
    view.addGestureRecognizer(twoFingerTap)

    // Single-finger pan (for mouse movement)
    let singlePan = UIPanGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.handleSinglePan))
    ...
    let twoFingerPan = UIPanGestureRecognizer(
      target: context.coordinator, action: #selector(Coordinator.handleTwoFingerPan))
